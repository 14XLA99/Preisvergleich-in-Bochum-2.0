...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const normalIcon = L.icon({
    ...
    popupAnchor: [1, -34], shadowUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-shadow.png',
    shadowSize: [41, 41]
  });
    ...
    stepContent.innerHTML = `
      ...
      <label>Preis (€):
        <input id="preisInput" type="number" step="0.01"
          value="${preisDaten[currentSupermarkt]?.preise?.[p.name] || ""}" />
      </label>
    `;
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) setTimeout(refreshMap, 100);
  });
